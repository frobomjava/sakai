...
public abstract class BaseSiteService implements SiteService, Observer
{
	...
	public void joinByUser(String id, String userId) throws IdUnusedException, PermissionException {
		...
		// sfoster9@uwo.ca
		// once joined, add the user to the specified join group and send an
		// email, if these join options are selected
		// if anything goes wrong with adding to a group or sending email, the
		// join should still finish
		try {
			// do the join
			// New Code
		...
		try {
			// add joining user to the site's join group; the delegate method
			// checks if there are any groups to join
			joinSiteDelegate.addJoinerToGroup(site);
		} catch (Exception e) {
			M_log.error(String.format("Unexpected exception joining user %s to group in site %s: ", user, id), e);
		}
