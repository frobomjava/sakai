...
$PBJQ(window).scroll(function(){var a=($window.height()-padding)>$tools.height();
var b=0;
if($PBJQ("#toggleSubsitesLink").length>0){b=(-1*($PBJQ("#toolMenu").height()-$PBJQ("#toggleSubsitesLink").position().top))
}if($PBJQ("#toolMenuWrap").css("position")!=="fixed"&&a){if($window.scrollTop()>offset.top){$PBJQ("#toolMenu").stop().animate({top:$window.scrollTop()-offset.top});
$PBJQ("#subSites.floating").css("top",($window.scrollTop()-offset.top+b))
}else{$PBJQ("#toolMenu").stop().animate({top:0});
$PBJQ("#subSites.floating").css("top",b)
}}else{$PBJQ("#subSites.floating").css("top",b)
}});
