console.log("in morpheus.scripts.min.scorm.js");
...
$PBJQ(document).ready(function () {
    updateFooterTime = (function () {
        if ($PBJQ("#preferredTime").length == 1) {
            var c = $PBJQ("#preferredTime").data("preferredtzdisplay");
...
var dhtml_view_sites = function () {
    ...
    dhtml_view_sites = function () {
        var e = $PBJQ("#selectSiteModal");
        e.show();
        if (e.hasClass("outscreen")) {
            ...
            if (a.length > 0) {
                ...
                if ($PBJQ("html").attr("dir") !== "rtl") {
                    e.css("top", c).css("right", f)
                } else {
                    e.css("top", c).css("left", $PBJQ("body").outerWidth() - f)
                }
            ...
            $PBJQ(".selectedTab").bind("click", function (g) {
                dhtml_view_sites();
                return false
            });
...
function closeDrawer() {
    ...
    $PBJQ("#portalMask").css("height", browserSafeDocHeight()).css({ width: "100%", "z-index": 1000, top: 0, left: 0 }).bind("click", function (b) {
        a();
        return false
    });
    ...
    if ($PBJQ("." + a).length) {
        ...
        var c = $PBJQ('<ul id="otherSiteTools" role="menu" />').addClass(g);
        ...
        $PBJQ.getJSON(f, function (j) {
            $PBJQ.each(j, function (l, m) {
                if (!m.tools[0]) {
                    return true
                } if (l < e) {
                    var k = i.clone();
                    ...
                    if (m.toolpopup) {
                        k.find("a").attr("href", m.tools[0].url + "?sakai.popup=yes").attr("onclick", "window.open(" + m.toolpopupurl + "); return false")
                    } c.append(k)
                }
            });
            if (j.length > e) {
    ...
    if (b) {
        if (portal.shortDescription) {
            b = b + " (" + portal.shortDescription + ")"
        } $PBJQ(".portletTitle h2").prepend('<span class="siteTitle">' + b + ":</span> ")
    } $PBJQ("#txtSearch").keyup(function (d) {
        if (d.keyCode == 27) {
            a()
        } if ($PBJQ("#txtSearch").val().length > 0) {
            var e = $PBJQ("#txtSearch").val().toLowerCase();
            $PBJQ(".fav-sites-term, .fav-sites-entry").hide();
            var c = $PBJQ(".fav-sites-entry").filter(function (f, g) {
                return ($PBJQ(".fav-title a", g).attr("title").toLowerCase().indexOf(e) >= 0)
            });
            c.show();
    ...
    $PBJQ("#presenceToggle").click(function (c) {
        c.preventDefault();
        $PBJQ("#presenceArea").toggle()
    });
    $PBJQ(".trayPopupClose").click(function (c) {
        c.preventDefault();
        $PBJQ(this).closest(".trayPopup").hide()
    });
    if ($PBJQ("a.tool-directurl").length) {
        $PBJQ("a.tool-directurl").cluetip({ local: true, arrows: true, cluetipClass: "jtip", sticky: true, cursor: "pointer", activation: "click", closePosition: "title", closeText: '<img src="/library/image/silk/cross.png" alt="close">' })
    }
});
$PBJQ(document).ready(function (i) {
    var a = [];
    var f = false;
    ...
    var j = {};
    $PBJQ(".site-favorite-btn", m).each(function (q, r) {
    j[$PBJQ(r).data("site-id")] = $PBJQ(r).parent()
    });
    ...
    var h = function (q) {
        $PBJQ.ajax({
            url: "/portal/favorites/list", method: "GET", dataType: "text", success: function (r) {
                a = r.split(";").filter(function (t, s) {
                    return t != ""
                });
                q(a)
            }
    ...
    var e = function (q, s) {
        ...
        if (s === "favorite") {
            $PBJQ(q).attr("title", $PBJQ("#removeFromFavoritesText").text())
        } else {
            if (s === "nonfavorite") {
                $PBJQ(q).attr("title", $PBJQ("#addToFavoritesText").text())
            } else {
                $PBJQ(q).attr("title", null)
            }
        } $PBJQ(q).empty().append($PBJQ(r.markup))
    };
    var d = function () {
        var q = $PBJQ(".site-favorite", m).length;
        $PBJQ(".favoriteCount", b).text("(" + q + ")");
        if (q < 2) {
            $PBJQ(".organizeFavorites", b).addClass("tab-disabled")
        } else {
            $PBJQ(".organizeFavorites", b).removeClass("tab-disabled")
        }
    };
    var n = function (q) {
        $PBJQ(".site-favorite-btn", m).each(function (r, s) {
            var t = $PBJQ(s).data("site-id");
            if ($PBJQ(s).closest(".my-workspace").length > 0) {
                e(s, "myworkspace")
            } else {
                if ($PBJQ.inArray(t, q) >= 0) {
                    e(s, "favorite")
                } else {
                    e(s, "nonfavorite")
                }
    ...
    var o = function () {
        return $PBJQ(".site-favorite-btn", m).has(".site-favorite").map(function () {
        ...
        q = q.sort(function (t, s) {
            if (a.indexOf(t) === -1) {
                ...
                if (a.indexOf(s) === -1) {
                    ...
                    return a.indexOf(t) - a.indexOf(s)
                }
            }
        });
    ...
    $PBJQ(m).on("click", ".site-favorite-btn", function () {
        var r = this;
    ...
    $PBJQ(b).on("click", ".tab-btn", function () {
    ...
    $PBJQ(b).on("tab-shown", function (s, q) {
        if (q === "organizeFavorites") {
            var r = $PBJQ("#organizeFavoritesList");
            r.empty();
            $PBJQ("#otherSiteTools").remove();
            $PBJQ("#organizeFavoritesPurgatoryList").empty();
            $PBJQ.each(a, function (u, w) {
                if (!j[w]) {
                    return
                } if ($PBJQ(j[w]).hasClass("my-workspace")) {
                    return
            ...
            r.sortable({
                stop: function () {
                    a = r.find(".organize-favorite-item").map(function () {
                        return $PBJQ(this).data("site-id")
                    }).toArray();
                    g()
                }
    ...
    $PBJQ(m).on("click", ".toolMenus", function (q) {
        q.preventDefault();
        showToolMenu($PBJQ(this));
        return false
    });
    $PBJQ(c).on("click", ".site-favorite-btn", function () {
        var s = this;
        if ($PBJQ(s).closest(".my-workspace").length > 0) {
            return
        } var q = $PBJQ(s).parent();
        var r;
        if ($PBJQ(s).closest("#organizeFavoritesList").length == 0) {
            $PBJQ("#organizeFavoritesList").append(q);
            r = "favorite"
        } else {
            $PBJQ("#organizeFavoritesPurgatoryList").append(q);
            r = "nonfavorite"
        } e(s, r);
...
var profile = profile || {};
profile.requestFriend = function (b, a) {
    $PBJQ.ajax({ url: "/direct/profile/" + b + "/requestFriend?friendId=" + a, dataType: "text", cache: false }).done(function (d, e, c) {
        $PBJQ("#profile-popup-unconnected-block-" + a).hide();
        $PBJQ("#profile-popup-requested-block-" + a).show()
    });
    return false
};
profile.confirmFriendRequest = function (b, a) {
    $PBJQ.ajax({ url: "/direct/profile/" + b + "/confirmFriendRequest?friendId=" + a, dataType: "text", cache: false }).done(function (d, e, c) {
        $PBJQ("#profile-popup-incoming-block-" + a).hide();
        $PBJQ("#profile-popup-connected-block-" + a).show()
    });
    return false
};
profile.removeFriend = function (a, b) {
    $PBJQ.ajax({ url: "/direct/profile/" + a + "/removeFriend?friendId=" + b, dataType: "text", cache: false }).done(function (d, e, c) {
        $PBJQ("#profile-popup-connected-block-" + b).hide();
        $PBJQ("#profile-popup-unconnected-block-" + b).show()
    });
    return false
};
profile.ignoreFriendRequest = function (a, b) {
    $PBJQ.ajax({ url: "/direct/profile/" + a + "/ignoreFriendRequest?friendId=" + b, dataType: "text", cache: false }).done(function (d, e, c) {
        $PBJQ("#profile-popup-requested-block-" + b).hide();
        $PBJQ("#profile-popup-incoming-block-" + b).hide();
        $PBJQ("#profile-popup-unconnected-block-" + b).show()
    });
    return false
};
function publishSite(b) {
    var a = "/direct/site/" + b + "/edit";
    var c = $PBJQ.ajax({
        type: "POST", data: "published=true", url: a, success: function () {
            location.reload()
        }
    }).responseText
...
} function toggleQuickLinksNav(b) {
    b.preventDefault();
    ...
    if (!$PBJQ(".Mrphs-quickLinksNav__subnav").hasClass("is-hidden")) {
        var a = $PBJQ('<div class="quicklinks-dropdown-overlay" />');
        a.on("click", function (c) {
            toggleQuickLinksNav(c)
        });
        $PBJQ("body").prepend(a);
...
var poll_session_data = function () {
    $PBJQ.ajax({
        url: "/direct/session/" + sessionId + ".json?auto=true&_=" + (new Date()).getTime(), dataType: "json", success: function (b) {
        b.maxInactiveInterval = b.maxInactiveInterval * 1000;
            if (b.active && b.userId != null && b.lastAccessedTime + b.maxInactiveInterval > b.currentTime) {
                var a = b.lastAccessedTime + b.maxInactiveInterval - b.currentTime;
                if (a < timeoutDialogWarningTime * 1000) {
                    min = Math.round(a / (1000 * 60));
...
function fetch_timeout_dialog() {
    $PBJQ.ajax({
        url: "/portal/timeout?auto=true", cache: true, dataType: "text", success: function (a) {
            timeoutDialogFragment = a
        }, error: function (c, a, b) {
            timeoutDialogEnabled = false
        }
    })
} function show_timeout_alert(b) {
    if (!timeoutDialogEnabled) {
        ...
        $PBJQ("#timeout_alert_body span").html(b)
    } else {
        var a = timeoutDialogFragment.replace("{0}", b);
        $PBJQ("body").append(a)
    }
} function toggleShortUrlOutput(b, c, a) {
    if ($PBJQ(c).is(":checked")) {
        $PBJQ.ajax({
            url: "/direct/url/shorten?path=" + encodeURI(b), dataType: "text", success: function (d) {
                $PBJQ("." + a).val(d)
            }
        })
    } else {
    ...
    $PBJQ(".Mrphs-toolTitleNav__link--directurl").click(function (b) {
        var a = $PBJQ(this).position();
        $PBJQ(this).siblings(".Mrphs-directUrl").toggleClass("active").css({ left: a.left + "px" });
        b.preventDefault()
    });
    $PBJQ(".Mrphs-directUrl .dropDown_close").click(function (a) {
...
$PBJQ(document).ready(function () {
    var a = 0;
    $PBJQ(document).scroll(function (c) {
        var b = $PBJQ(this).scrollTop();
        if (b > a && b > 90) {
...
var buildDropdownMenu = function (b, a, g) {
    var i = '<ul class="Mrphs-sitesNav__submenu" role="menu">';
    ...
    $PBJQ.ajax({
        url: f, dataType: "json", success: function (j) {
            $PBJQ.each(j, function (n, o) {
                ...
                if (h == o.tools[0].id) {
                    ...
                    var l;
                    if (o.toolpopup) {
                        var k = ' role="menuitem" href="{{tool_url}}?sakai.popup=yes" title="{{item_title}}" onclick="window.open(\'{{item_toolpopupurl}}\');"';
                        ...
                        var k = ' role="menuitem" href="{{tool_url}}" title="{{item_title}}"';
...
var setupSiteNav = function () {
    ...
    $PBJQ(document).on("keydown", ".Mrphs-sitesNav__menu > li.Mrphs-sitesNav__menuitem > a", function (a) {
        if (a.keyCode == 40) {
            a.preventDefault();
            var b = $PBJQ(this).parent().find(".Mrphs-sitesNav__dropdown");
            if (b.data("clicked")) { } else {
                b.data("clicked", true);
    ...
    $PBJQ("ul.Mrphs-sitesNav__menu li span.Mrphs-sitesNav__dropdown").click(function (f, b) {
        f.preventDefault();
        var h = $PBJQ(f.target);
        ...
        var c = function (i) {
            ...
            if (b) {
                a.find("a.Mrphs-sitesNav__submenuitem-title").first().focus()
        ...
        if (!a.find("ul").length) {
            buildDropdownMenu(a, h.attr("data-site-id"), c)
        } else {
            c(a.find("ul"))
        }
    }).hover(function () {
...
function addArrowNavAndDisableTabNav(a) {
    a.find("li a").attr("tabindex", "-1").keydown(function (f) {
        var d = $PBJQ(f.target);
        if (f.keyCode == 40) {
            f.preventDefault();
            var b = d.closest("li").next();
            if (b.length === 0 || b.find("a.Mrphs-sitesNav__submenuitem-title").length == 0) {
                b = a.find("li").first()
            ...
            if (f.keyCode == 38) {
                ...
                if (c.length === 0) {
                    c = a.find("a.Mrphs-sitesNav__submenuitem-title").closest("ul")
    ...
    if (a.text() == b.data("title-expand") || collapsed) {
...
//var $window = $PBJQ(window), offset = $PBJQ("#toolMenu").offset(), $tools = $PBJQ("#toolMenu"), padding = $PBJQ(".Mrphs-siteHierarchy").height() + $PBJQ(".Mrphs-topHeader").height();
/*$PBJQ(document).ready(function () {
    ...
    $PBJQ(window).scroll(function () {
        var a = ($window.height() - padding) > $tools.height();
        var b = 0;
        if ($PBJQ("#toggleSubsitesLink").length > 0) {
            b = (-1 * ($PBJQ("#toolMenu").height() - $PBJQ("#toggleSubsitesLink").position().top))
        } if ($PBJQ("#toolMenuWrap").css("position") !== "fixed" && a) {
            if ($window.scrollTop() > offset.top) {
                $PBJQ("#toolMenu").stop().animate({ top: $window.scrollTop() - offset.top });
                $PBJQ("#subSites.floating").css("top", ($window.scrollTop() - offset.top + b))
            } else {
                $PBJQ("#toolMenu").stop().animate({ top: 0 });
                $PBJQ("#subSites.floating").css("top", b)
            }
        } else {
            $PBJQ("#subSites.floating").css("top", b)
        }
    });
    $PBJQ("#toggleSubsitesLink").click(function (c) {
        var b = $PBJQ(this);
...
function getCookieVal(d) {
    ...
    for (var b = 0;
        b < c.length;
        ++b) {
            var a = (c[b].substring(0, c[b].indexOf("="))).trim();
        if (a === d) {
            return ((c[b].split("="))[1]).trim()
        }
    } return undefined
} function updatePresence() {
    $PBJQ.ajax({
        url: sakaiPresenceFragment, cache: false, success: function (g) {
            var f = g.indexOf("<ul");
            if (f < 1) {
            ...
            for (var d = 0;
                d < e.length;
                d++) {
                    if (b == e.substr(d, b.length)) {
                        a++
                    }
            } if (a > 1) {
                $PBJQ("#presenceCount").html(a + "");
...
} function toggleUserNav(b) {
    b.preventDefault();
    $PBJQ(".Mrphs-userNav__subnav").toggleClass("is-hidden");
    if (!$PBJQ(".Mrphs-userNav__subnav").hasClass("is-hidden")) {
        var a = $PBJQ('<div class="user-dropdown-overlay" />');
        a.on("click", function (c) {
            toggleUserNav(c)
        });
        $PBJQ("body").prepend(a);
...
$PBJQ(document).ready(function () {
    if ($PBJQ("#loginLink2").length == 1) {
        $PBJQ("#loginLink2").click(function (a) {
